...
<body>
                        ...
                        <div class="table-container">
                            <table class="customer-table">
                                ...
                                <tbody id="customerTableBody">
                                    <tr><td colspan="7" style="text-align:center;padding:30px;">Loading customers...</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Calculate points as user types
        ...
        async function loadCustomers() {
            try {
                ...
                if (data.success) {
                    ...
                    console.log('Loaded customers:', customers); // Debug
                    updateStats();
                    renderCustomerTable();
                } else {
        ...
        // Show voucher notification modal
        function showVoucherNotification(message, voucher) {
            const modal = document.getElementById('voucherModal');
            document.getElementById('voucherModalHeader').textContent = '🎉 Transaction Successful!';
            
            let body = `<p style="font-size:16px;margin:20px 0;">${message}</p>`;
            if (voucher) {
                body += `<div style="background:#28a745;color:#fff;padding:15px;border-radius:8px;margin:20px 0;">
                            <strong style="font-size:18px;">${voucher}</strong>
                         </div>`;
            }
            body += `<button onclick="closeVoucherModal()" style="background:#638ECB;color:#fff;padding:10px 30px;border:none;border-radius:5px;cursor:pointer;font-weight:600;">OK</button>`;
            
            document.getElementById('voucherModalBody').innerHTML = body;
        ...
        document.getElementById('loyaltyForm').addEventListener('submit', async function(e) {
            ...
            formData.append('purchase_amount', document.getElementById('purchaseAmount').value);
        ...
        async function redeemVoucher(id, voucherType) {
            if (!confirm(`Redeem ${voucherType} discount voucher? This will reset points to 0.`)) return;
        ...
        function printCustomers() {
            ...
            printWindow.document.write('<html><head><title>Loyalty Customers</title>');
            printWindow.document.write('<style>body{font-family:Arial;} table{width:100%;border-collapse:collapse;} th,td{border:1px solid #ddd;padding:8px;text-align:left;} th{background:#638ECB;color:#fff;}</style>');
            printWindow.document.write('</head><body>');
            printWindow.document.write('<h1>Panyeros Kusina - Loyalty Customers</h1>');
            printWindow.document.write('<p>Generated: ' + new Date().toLocaleString() + '</p>');
            printWindow.document.write('<table><thead><tr><th>Name</th><th>Contact</th><th>Points</th><th>Expiry Date</th><th>Last Activity</th></tr></thead><tbody>');
            
            filteredCustomers.forEach(c => {
                printWindow.document.write(`<tr><td>${c.name}</td><td>${c.contact}</td><td>${c.points}</td><td>${c.expdate}</td><td>${c.timestamp}</td></tr>`);
            });
        ...
        function copyCustomerData() {
            ...
            let text = 'Name\tContact\tPoints\tExpiry Date\tLast Activity\n';
            filteredCustomers.forEach(c => {
                text += `${c.name}\t${c.contact}\t${c.points}\t${c.expdate}\t${c.timestamp}\n`;
            });
            
            navigator.clipboard.writeText(text).then(() => {
                showAlert('✅ Customer data copied to clipboard!', 'success');
            }).catch(() => {
